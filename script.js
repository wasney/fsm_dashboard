...
// *** NEW: Store Details Section Reference ***
...
function populateFiltersOnInit(data) {
    ...
    const requiredFilterCols = ['SUB_CHANNEL', 'Q2 Territory', 'Store']; // Columns needed for filters
    ...
    updateDropdownOptions(territoryFilter, [...territories].sort(), 'Territory'); // Populate Territory first
    ...
    // Subchannel is single select, handled by updateDropdownOptions
...
// This function might need refinement if stores should be filtered by Territory *and* Subchannel
function updateStoreFilterOptions() {
    const selectedSubchannel = subchannelFilter.value;
    // TODO: Optionally get selectedTerritories here as well if needed
    const relevantStores = new Set();
    let dataToScan = originalData;
    // Filter by Subchannel first
    if (selectedSubchannel !== 'ALL') {
        dataToScan = dataToScan.filter(row => String(row['SUB_CHANNEL'] || '').trim() === selectedSubchannel);
    }
    // TODO: Optionally filter dataToScan by selected Territories here
    // const selectedTerritories = Array.from(territoryFilter.selectedOptions).map(o => o.value).filter(v => v !== 'ALL');
    // if (selectedTerritories.length > 0) {
    //    dataToScan = dataToScan.filter(row => selectedTerritories.includes(String(row['Q2 Territory'] || '').trim()));
    // }

    if (dataToScan.length > 0 && dataToScan[0]['Store'] !== undefined) {
        dataToScan.forEach(row => { if (row['Store'] && String(row['Store']).trim()) relevantStores.add(String(row['Store']).trim()); });
    } else if (originalData.length > 0 && (!originalData[0] || originalData[0]['Store'] === undefined)) {
        if (!statusElement.textContent.includes("Warning: 'Store' column")) statusElement.textContent += ` Warning: 'Store' column may be missing, Store filter cannot be updated.`;
    }
...
function filterAndDisplayData() {
    ...
    const selectedTerritoryOptions = Array.from(territoryFilter.selectedOptions);
    let selectedTerritories = selectedTerritoryOptions.map(option => option.value);
    const allTerritoriesOption = territoryFilter.querySelector('option[value="ALL"]');
    const containsAllTerr = selectedTerritories.includes('ALL');
    const containsIndividualTerr = selectedTerritories.some(val => val !== 'ALL');
    ...
    let selectedStores = selectedStoreOptions.map(option => option.value);
    const allStoresOption = storeFilter.querySelector('option[value="ALL"]');
    const containsAllStores = selectedStores.includes('ALL');
    const containsIndividualStores = selectedStores.some(val => val !== 'ALL');
    ...
    const isAllStoresSelected = selectedStores.includes('ALL'); // Final check
    const isSingleStoreSelected = !isAllStoresSelected && selectedStores.length === 1; // Check based on final store selection

    // --- Apply Filters ---
    let fullyFilteredData = originalData;
    // Filter order: Subchannel -> Territory -> Store
    ...
    const baseMessage = `Displaying ${fullyFilteredData.length} matching records for summary from all ${originalData.length} data rows.`; // Updated row count text
    let currentStatus = statusElement.textContent;
    let warningPart = "";
    if(currentStatus.includes("Warning:")) { warningPart = currentStatus.substring(currentStatus.indexOf("Warning:")); }
...
function processAndDisplayData(fullyFilteredData, subchannelFilteredData, isSingleStoreSelected, isAllStoresSelected) {
    ...
    let districtValueForDisplay = null; // New
    let postTrainValueForDisplay = null;
    let storeDetailsData = null; // New for store details section

    // Flags for conditional display of combined %
    let showCombinedTerritoryRev = false;
    let showCombinedDistrictRev = false;

    // Define all required columns - ADD NEW ONES HERE
    ...
    const districtCols = ['District', 'District Rev%']; // Assuming these names
    ...
    const allRequiredCols = [...new Set([...baseRequiredCols, ...districtCols, ...storeDetailCols, ...attachRateColumns])]; // Combine and remove duplicates
    ...
    if (dataToCheckColumns.length > 0) {
        allRequiredCols.forEach(col => {
            if (dataToCheckColumns[0][col] === undefined) {
                missingCols.push(col);
            }
        });
    } else if (originalData.length > 0) { // If filtered is empty but original isn't, check original
         allRequiredCols.forEach(col => {
            if (originalData[0][col] === undefined) {
                missingCols.push(col);
            }
        });
    }
    if (missingCols.length > 0) {
        console.warn("Required columns missing:", missingCols);
        if (!statusElement.textContent.includes('Warning:')) statusElement.textContent += ' Warning: ';
        statusElement.textContent += ` Missing columns (${missingCols.slice(0,3).join(', ')}${missingCols.length > 3 ? '...' : ''}). Some sections/calculations may fail.`;
    }

    // --- Process FULLY Filtered Data ---
    if (fullyFilteredData.length > 0 && missingCols.length === 0) { // Only proceed if core cols likely present
        ...
        // Determine Territory & District Values and Combined Flags
        if (isSingleStoreSelected && fullyFilteredData.length === 1) {
            ...
            districtValueForDisplay = parsePercentage(singleRow['District Rev%']); // Get single district value
            // Show single labels
            showCombinedTerritoryRev = false;
            showCombinedDistrictRev = false;

            // --- Populate Store Details Data ---
            storeDetailsData = {};
            storeDetailCols.forEach(col => storeDetailsData[col] = singleRow[col]); // Copy all needed details
            // Also copy Store and Q2 Territory as they are needed for display logic
            storeDetailsData['Store'] = singleRow['Store'];
            storeDetailsData['Q2 Territory'] = singleRow['Q2 Territory'];

        } else if (!isAllStoresSelected && fullyFilteredData.length > 1) {
            // Check if all selected stores share the same Territory
            ...
            // Check if all selected stores share the same District
            ...
            // Calculate Territory Sum only if all are in the same territory
            if (allSameTerritory && firstTerritory !== '') {
                let sumT = 0; let countT = 0;
                fullyFilteredData.forEach(row => { const val = parsePercentage(row['Territory Rev%']); if (val !== null) { sumT += val; countT++; } });
                territoryValueForDisplay = countT > 0 ? sumT : null;
                showCombinedTerritoryRev = true;
            } else {
                territoryValueForDisplay = null; // Hide if different territories selected
                showCombinedTerritoryRev = false;
            }

            // Calculate District Sum only if all are in the same district
            if (allSameDistrict && firstDistrict !== '') {
                 let sumD = 0; let countD = 0;
                fullyFilteredData.forEach(row => { const val = parsePercentage(row['District Rev%']); if (val !== null) { sumD += val; countD++; } });
                districtValueForDisplay = countD > 0 ? sumD : null;
                showCombinedDistrictRev = true;
            } else {
                districtValueForDisplay = null; // Hide if different districts selected
                showCombinedDistrictRev = false;
            }
             // Ensure store details are hidden when multiple stores are selected
             storeDetailsData = null;

        } else { // All stores selected or only one store matched implicitly (treat as combined)
             territoryValueForDisplay = null; // Hide combined totals when "All Stores" is effectively selected
             districtValueForDisplay = null;
             showCombinedTerritoryRev = false;
             showCombinedDistrictRev = false;
             storeDetailsData = null;
        }

        // Determine Post Training Score display value
    ...
    // --- 5. Display Sections ---
    displayStoreDetails(storeDetailsData); // Display store details (or hide section)
    displaySummary( totalQtdTargetFiltered, totalRevenueWithDFFiltered, overallRevenueAchievementFilteredDecimal, territoryRevARUnfilteredDecimal, isSingleStoreSelected, territoryValueForDisplay, showCombinedTerritoryRev, districtValueForDisplay, showCombinedDistrictRev, // Pass new values/flags
    ...
    const dataForDetails = isAllStoresSelected ? subchannelFilteredData : fullyFilteredData; // Use the correct data source

    if (dataForDetails.length > 0) {
        if (missingCols.length === 0 || !(missingCols.includes('Tablet Attach Rate'))) { // Check if needed columns are missing
            attachRateColumns.forEach(colName => { attachRateStats[colName] = calculateColumnStats(dataForDetails, colName); });
        } else {
        ...
        dataForDetails.forEach((row, index) => {
            ...
            if (missingCols.length === 0 || !(missingCols.includes('Rev AR%') || missingCols.includes('Unit Achievement'))) {
             ...
             if (missingCols.length === 0 || !(missingCols.includes('Tablet Attach Rate'))) { // Check specific table column
    ...
    const chartData = (lineChartLabels.length > 0)
...
// *** NEW Function: displayStoreDetails ***
function displayStoreDetails(details) {
    if (!storeDetailsElement) return; // Safety check

    if (details) {
        let html = `<h3>Store Details</h3>`;
        const addDetail = (label, value) => {
            if (value !== null && value !== undefined && String(value).trim() !== '') {
                 html += `<p><strong>${label}:</strong> ${String(value).trim()}</p>`;
            }
        };
        const addConditionalDetail = (label, value, conditionVal = 'yes') => {
             if (value !== null && value !== undefined && String(value).trim().toLowerCase() === conditionVal.toLowerCase()) {
                  html += `<p><strong>${label}:</strong> Yes</p>`; // Display Yes if condition met
             }
             // Optionally add an 'else' here if you want to show "No" or hide the line completely
        };
        ...
        addDetail('Phone #', details['PHONE_NO']); // Add only if not blank handled by addDetail

        // Previous Territory (only if different)
        const q1Terr = String(details['Q1 TERRITORY'] || '').trim();
        const q2Terr = String(details['Q2 Territory'] || '').trim();
        if (q1Terr && q2Terr && q1Terr !== q2Terr) {
            addDetail('Previous Territory', q1Terr);
        }

        // Q2 Territory with FSM Name
        const fsmName = String(details['FSM NAME'] || '').trim();
        const q2Display = fsmName ? `${q2Terr} (${fsmName})` : q2Terr;
        addDetail('Q2 Territory', q2Display);

        // Dealer Name (conditional)
        if (String(details['CHANNEL_TYPE'] || '').toLowerCase() === 'dealer') {
             addDetail('Dealer Name', details['DEALER_NAME']);
        }
        // CinglePoint ID (conditional)
        if (String(details['CHANNEL'] || '').toLowerCase() === 'at&t') { // Assuming 'AT&T' check
             addDetail('CinglePoint ID', details['CINGLEPOINT_ID']);
        }

        addConditionalDetail('Super Store', details['SUPER STORE']);
        addConditionalDetail('Golden Rhino', details['GOLDEN RHINO']);
        addConditionalDetail('Hispanic Market', details['Hispanic_Market']);
        addConditionalDetail('GCE', details['GCE']);
        addConditionalDetail('AI Zone', details['AI_Zone']);

        addDetail('Latitude', details['LATITUDE_ORG']);
        addDetail('Longitude', details['LONGITUDE_ORG']);
        addDetail('Rank', details['RETAIL_MAP_OPP_TIER_NAME']);


        storeDetailsElement.innerHTML = html;
        storeDetailsElement.style.display = 'block';
        summaryElement.style.marginTop = '0'; // Adjust summary margin
    } else {
        storeDetailsElement.innerHTML = ''; // Clear content
        storeDetailsElement.style.display = 'none'; // Hide section
        summaryElement.style.marginTop = '1rem'; // Restore summary margin
    }
}


// *** UPDATED displaySummary parameters and logic ***
function displaySummary(
    ...
    isSingleStoreSelected, // Flag for single store
    territoryValueDecimal, showCombinedTerritoryRev, // Territory value and flag
    districtValueDecimal, showCombinedDistrictRev,   // District value and flag
    ...
    // Determine Territory Label and Value string
    let territoryHtml = '';
    if (isSingleStoreSelected) {
        territoryHtml = `<p><strong>Territory Rev %:</strong> ${formatPercentage(territoryValueDecimal, 2)}</p>`;
    } else if (showCombinedTerritoryRev) {
        territoryHtml = `<p><strong>Combined Territory Rev %:</strong> ${formatPercentage(territoryValueDecimal, 2)}</p>`;
    } // Else: Hide the line

     // Determine District Label and Value string
    let districtHtml = '';
    if (isSingleStoreSelected) {
        districtHtml = `<p><strong>District Rev %:</strong> ${formatPercentage(districtValueDecimal, 2)}</p>`;
    } else if (showCombinedDistrictRev) {
        districtHtml = `<p><strong>Combined District Rev %:</strong> ${formatPercentage(districtValueDecimal, 2)}</p>`;
    } // Else: Hide the line


    const displayUnitAchPercent = overallUnitAchievementFilteredPercent !== null ? overallUnitAchievementFilteredPercent.toFixed(2) + '%' : 'N/A';

    summaryElement.innerHTML = `
        <h3>Filtered QTD Summary</h3>
        <p><strong>Total QTD Revenue Target:</strong> ${formatCurrency(totalQtdTargetFiltered)}</p>
        <p><strong>Total Revenue w/DF (QTD):</strong> ${formatCurrency(totalRevenueWithDFFiltered)}</p>
        <p><strong>Overall QTD Revenue Achievement:</strong> <span style="background-color: ${achievementBgColor}; padding: 2px 5px; border-radius: 3px;">${displayRevenueAchPercent}</span> ${territoryRevARUnfilteredDecimal !== null ? ` (Territory Benchmark: ${formatPercentage(territoryRevARUnfilteredDecimal, 2)})` : ''}</p>
        ${territoryHtml} <!-- Insert Territory HTML -->
        ${districtHtml}  <!-- Insert District HTML -->
        <hr style="border-top: 1px solid #444; margin: 10px 0;">
        <p><strong>Total Units w/ DF:</strong> ${totalUnitsWithDFFiltered.toLocaleString()}</p>
        <p><strong>Total Unit Target:</strong> ${totalUnitTargetFiltered.toLocaleString()}</p>
        <p><strong>Overall Unit Achievement:</strong> ${displayUnitAchPercent}</p>
    `;
    ...
    if (!hasSummaryData && originalData.length > 0 && fullyFilteredData.length > 0) { // Check fullyFilteredData too
    ...
    } else if (fullyFilteredData.length === 0 && originalData.length > 0) {
         // Message already handled by status element, keep summary clear or show placeholder
...
function clearDashboard() {
    ...
    if (storeDetailsElement) {
        ...
        summaryElement.style.marginTop = '1rem'; // Restore margin
    }
    ...
    subchannelFilter.innerHTML = '<option value="ALL">-- Load File First --</option>';
    subchannelFilter.disabled = true;
    subchannelFilter.value = 'ALL';
    storeFilter.innerHTML = '<option value="ALL">-- Load File First --</option>';
    resetMultiSelect(storeFilter, 'ALL');
    storeFilter.disabled = true;
    originalData = [];
    // Clear Table & Reset Title
    attachRateTableBody.innerHTML = '';
    attachTableStatusElement.textContent = '';
    if(attachRateTableHeading) attachRateTableHeading.textContent = 'Attach Rates'; // Reset generic title
    // Clear Conditional Sections
    if(repPmrSection) repPmrSection.style.display = 'none';
    if(trainingStatsSection) trainingStatsSection.style.display = 'none';
    if(eliteP) eliteP.style.display = 'none';
    if(eliteValue) eliteValue.textContent = 'N/A';
    if(postTrainingScoreValue) postTrainingScoreValue.textContent = 'N/A';
    if(repSkillAchValue) repSkillAchValue.textContent = 'N/A';
    if(vPmrAchValue) vPmrAchValue.textContent = 'N/A';
    // Clear Email Share
