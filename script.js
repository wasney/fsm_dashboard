...
function filterAndDisplayData() {
    ...
    let selectedStores = selectedStoreOptions.map(option => option.value); // Get initially selected values
    ...
    if (containsAll && containsIndividual) {
        // If 'ALL' and individual stores are selected, deselect 'ALL'
        if (allStoresOption) allStoresOption.selected = false;
        selectedStores = selectedStores.filter(val => val !== 'ALL'); // Update array
    } else if (!containsAll && selectedStores.length === 0 && allStoresOption) {
        // If *nothing* is selected, default back to selecting 'ALL'
        allStoresOption.selected = true;
        selectedStores = ['ALL']; // Update array
    } else if (containsAll && !containsIndividual && selectedStores.length === 1 && allStoresOption) {
        // If only 'ALL' is selected, ensure it's visually selected
        if (!allStoresOption.selected) allStoresOption.selected = true;
        // Deselect others (redundant but safe)
        for(let i = 0; i < storeFilter.options.length; i++) {
            if (storeFilter.options[i].value !== 'ALL') {
                storeFilter.options[i].selected = false;
            }
        }
    }
    // --- End of Smart Deselection Logic ---


    // Determine filter flags based on the *potentially updated* selectedStores array
    ...
    // Filter for subchannel only (for table/chart)
    ...
    if(currentStatus.includes("Warning:")) {
        warningPart = currentStatus.substring(currentStatus.indexOf("Warning:"));
    }
    if (fullyFilteredData.length === 0 && originalData.length > 0) {
        statusElement.textContent = "No data matches the current Subchannel/Store filter selection." + warningPart;
    } else if (originalData.length > 0) {
         statusElement.textContent = baseMessage + warningPart;
    }

    // Pass the isSingleStoreSelected flag to the processing function
    processAndDisplayData(fullyFilteredData, subchannelFilteredData, isSingleStoreSelected);
}
...
function processAndDisplayData(fullyFilteredData, subchannelFilteredData, isSingleStoreSelected) {
    ...
    if (fullyFilteredData.length > 0 && missingSummaryCols.length === 0) {
        ...
        if (isSingleStoreSelected && fullyFilteredData.length === 1) {
            ...
            let sumOfTerritoryRevPercentages = 0;
            let countValidTerritoryRev = 0;
            fullyFilteredData.forEach(row => {
                 const terrRev = parsePercentage(row['Territory Rev%']);
                 if (terrRev !== null) {
                     sumOfTerritoryRevPercentages += terrRev;
                     countValidTerritoryRev++;
                 }
             });
             territoryValueForDisplay = countValidTerritoryRev > 0 ? sumOfTerritoryRevPercentages : null;
             isSingleStoreSelected = false;
        }
    }
...
// Add this block at the end of your script.js
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/sw.js') // Ensure path is correct (relative to origin)
      .then(registration => {
