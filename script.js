...
// *** NEW: Territory Filter Reference ***
...
function populateFiltersOnInit(data) {
    ...
    // *** UPDATED: Check for 'SUB_CHANNEL' and 'Q2 Territory' ***
    if (data.length === 0 || !data[0] ||
        data[0]['SUB_CHANNEL'] === undefined ||
        data[0]['Q2 Territory'] === undefined || // Added check
        data[0]['Store'] === undefined)
    {
        if(data.length > 0) {
            // *** UPDATED: Warning message ***
            statusElement.textContent += ` Warning: Could not fully populate filters. 'SUB_CHANNEL', 'Q2 Territory', or 'Store' column might be missing in first 61 data rows.`;
        }
    ...
    // *** NEW: Populate Territory Filter ***
    updateDropdownOptions(territoryFilter, [...territories].sort(), 'Territory');
    updateDropdownOptions(storeFilter, [...stores].sort(), 'Store');

    resetMultiSelect(territoryFilter, 'ALL'); // Reset Territory filter
    resetMultiSelect(storeFilter, 'ALL'); // Reset Store filter

    subchannelFilter.disabled = !columnsPresent || subchannels.size === 0;
    // *** NEW: Disable Territory Filter appropriately ***
...
// This function currently only updates stores based on subchannel.
// We could enhance it to also consider territory, but keeping it simple for now.
function updateStoreFilterOptions() {
    ...
    if (selectedSubchannel !== 'ALL') {
        dataToScan = originalData.filter(row => String(row['SUB_CHANNEL'] || '').trim() === selectedSubchannel);
    }
    // TODO: Potentially filter dataToScan further based on selected territory here if desired.
    ...
    resetMultiSelect(storeFilter, 'ALL'); // Reset store selection when subchannel changes
}
...
function handleSubchannelChange() {
    updateStoreFilterOptions(); // Update available stores
    // When subchannel changes, we might want to reset territory too? Optional.
    // resetMultiSelect(territoryFilter, 'ALL');
    filterAndDisplayData(); // Refresh data based on new subchannel & reset filters
}
...
function filterAndDisplayData() {
    ...
    // 1. Filter by Subchannel
    if (selectedSubchannel !== 'ALL') {
        fullyFilteredData = fullyFilteredData.filter(row => String(row['SUB_CHANNEL'] || '').trim() === selectedSubchannel);
    }
    // 2. Filter by Territory
    if (!isAllTerritoriesSelected) {
        fullyFilteredData = fullyFilteredData.filter(row =>
            selectedTerritories.includes(String(row['Q2 Territory'] || '').trim())
        );
    }
    // 3. Filter by Store
    ...
    // Prepare subchannel-only data (still used if 'All Stores' is selected for chart/table)
    let subchannelFilteredData = originalData;
    if (selectedSubchannel !== 'ALL') {
        subchannelFilteredData = subchannelFilteredData.filter(row => String(row['SUB_CHANNEL'] || '').trim() === selectedSubchannel);
    }
...
// Accepts isAllStoresSelected flag now
function processAndDisplayData(fullyFilteredData, subchannelFilteredData, isSingleStoreSelected, isAllStoresSelected) {
    ...
    // Column Names (assuming they match the provided header list)
    ...
    // Check for Missing Columns based on first row of fullyFilteredData if available
    const dataToCheckColumns = fullyFilteredData.length > 0 ? fullyFilteredData : (subchannelFilteredData.length > 0 ? subchannelFilteredData : originalData); // Use appropriate data for check
    ...
    if (missingLineCols.length > 0) console.warn("Missing columns for Chart:", missingLineCols);
    if (missingTableCols.length > 0) console.warn("Missing columns for Attach Rate Table:", missingTableCols);

    // Process FULLY Filtered Data (for Summary/Conditional Stats)
    ...
    // Display Summary and Conditional Sections
    ...
    // Process Data for Chart and Table using appropriate dataset
    const dataForDetails = isAllStoresSelected ? subchannelFilteredData : fullyFilteredData;

    if (dataForDetails.length > 0) {
        if (missingTableCols.length === 0) { attachRateColumns.forEach(colName => { attachRateStats[colName] = calculateColumnStats(dataForDetails, colName); }); }
    ...
    if(missingTableCols.length === 0 && attachRateDataForTable.length > 0){ displayAttachRateTable(attachRateDataForTable, attachRateStats, isAllStoresSelected); }
...
function clearDashboard() {
    ...
    territoryFilter.innerHTML = '<option value="ALL">-- Load File First --</option>'; // Reset territory filter
    resetMultiSelect(territoryFilter, 'ALL');
    ...
    originalData = [];
    // Clear table and reset title
    attachRateTableBody.innerHTML = '';
    attachTableStatusElement.textContent = '';
    if(attachRateTableHeading) attachRateTableHeading.textContent = 'Attach Rates (Subchannel Filtered)'; // Reset table title
